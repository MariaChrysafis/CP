...
modint binPow (modint x, int64_t y) {
    assert(y >= 0);
    y %= (MOD - 1);
    modint ans = 1, res = x;
    while (y > 0) {
...
modint frac (modint a, modint b) {
    return a * binPow(b, MOD - 2);
}
bigint modulo = bigint(MOD - 1);
modint arithmetic (modint a, bigint n) {
    assert(n >= 0);
    if (n == 0) return 1;
    if (a == 1) return (stoll(bigint(n % MOD).to_string()) + 1) % MOD;
    if (a == 0) return 1;
    return frac(binPow(a, stoll(to_string(((n + MOD) % (MOD - 1))))) - 1, a - 1);
}
modint a, b, c, d;
bigint N, M;
modint f (bigint &i, bigint &j) {
    if (i.to_string() == "1" && j.to_string() == "1") return 1;
    int64_t new_i = stoll((bigint((i + MOD - 1) % (MOD - 1)) + bigint(MOD - 1)).to_string()), new_j = stoll((bigint((j + MOD - 1) % (MOD - 1)) + bigint(MOD - 1)).to_string());
    if (j.to_string() != "1") {
        bigint bs = bigint("1");
        return binPow(a, new_j - 1) * f(i, bs) + b * arithmetic(a, j - 2);
    }
    modint new_a = binPow(a, stoll(((M - 1) % bigint(MOD - 1)).to_string())) * c;
    modint new_b = c * b * arithmetic(a, M - 2) + d;
    return binPow(new_a, new_i - 1) + new_b * arithmetic(new_a, i - 2);
}
int main() {
    //freopen("cbarn.out", "r", stdin);
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    string str1, str2;
    cin >> str1 >> str2;
    N = bigint(str1), M = bigint(str2);
    cin >> a.n >> b.n >> c.n >> d.n;
    cout << f(N, M);
}
